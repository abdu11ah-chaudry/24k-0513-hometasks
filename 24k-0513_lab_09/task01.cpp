...
int main(){
    Node* root=nullptr;
    //First Inserting the already present records:
    int arr[5]={10,20,30,40,50};
    for(int i=0; i<5; i++){
        root=insert(root,arr[i]);
    }

    //Now insrting 15 and the balance that AVL would perform:
    root=insert(root,15);

    //Printing the balanced tree after insertion:
    cout<<"Inorder traversal Of AVL TREE: "<<endl;
    InOrderTraversal(root);

    cout<<endl<<"Height of the tree: "<<root->height<<endl;
